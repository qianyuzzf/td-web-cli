...
编译后的代码默认输出到 `dist/` 目录。
每次构建前会自动清理旧的 `dist/`，避免已删除源码对应的历史文件进入发布包。
...
发布前要求 Git 工作区不存在未提交修改，且新版本号必须高于当前版本号。

该命令将依次执行：

- 同步更新 `package.json` 和 `package-lock.json` 版本号
- 清理旧构建产物并重新构建
- 使用 `npm pack --dry-run` 检查实际发布内容
- 仅提交版本文件并推送 Git 代码
- Git 推送成功后发布到 npm
